...
**Correction 20052021
*Calcul âge moyen des grands parents
**Groupe d'âge de la mère (age of mother at birth of child)

capture drop pgf_age_birth
gen pgf_age_birth = (DoB - DoBPGF)/31557600000
*Age moyen
sum pgf_age_birth if pgFatherId!=""
*72.22837
capture drop pgm_age_birth
gen pgm_age_birth = (DoB - DoBPGM)/31557600000
sum pgm_age_birth if pgMotherId!=""
*63.37731  
capture drop mgf_age_birth
gen mgf_age_birth = (DoB - DoBMGF)/31557600000
sum mgf_age_birth if mgFatherId!=""
*64.00444
capture drop mgm_age_birth
gen mgm_age_birth = (DoB - DoBMGM)/31557600000
sum mgm_age_birth if mgMotherId!=""
*54.03455
...
bys Pgrandparents : stdes
bys Mgrandparents : stdes
...
plot1opts(lpattern(dash) lcolor(red)) /// this forces the first line to be dashed and red
plot2opts(lpattern(solid) lcolor(blue)) 
graph save "Figure (mère) 1 110521",replace 

graph export "Figure (mère) 1 110521.png", replace width(2000)
graph export "Figure (mère) 1 110521.tif", replace width(2000)
...
graph save "Figure (père) 2 110521",replace 

graph export "Figure (père) 2 110521.png", replace width(2000)
graph export "Figure (père) 2 110521.tif", replace width(2000)

graph combine "Figure (mère) 1 110521.gph" "Figure (père) 2 110521.gph", row(1) ///
note("Source: données des observatoires; calcul des auteurs.", span size(vsmall))

grc1leg "Figure (mère) 1 110521.gph" "Figure (père) 2 110521.gph", row(1) ring(0) ///
note("Source: données des observatoires; calcul des auteurs.", span size(vsmall))
...
*Modif 11052021
*sts test coresidMO, logrank
sts graph , hazard tmax(60) kernel(rectangle) width(1) by(coresidMO)
sts graph , hazard tmax(60) ci kernel(rectangle) ///
			width(4 4) by(coresidMO)
sts list, at(0(6)60) 
sts list, at(0(6)60)  failure by(coresidMO)
...
plot2opts(lpattern(solid) lcolor(blue)) ///
note("Source: HDSS data; authors’ calculations.", span size(vsmall)) 
graph save "Figure (mère) 2 110521",replace 

graph export "Figure (mère) 2 110521.png", replace width(2000)
graph export "Figure (mère) 2 110521.tif", replace width(2000
...
xla(0(10)60) /// X-label values! From 0 to 6 years with 1 year intervals in between
    legend(order (1 "GMM corésident" 2 "GMM non corésident   -   (log-rank p = 0,047)"  ///
        3 "GPM corésident" 4 "GPM non corésident   -   (log-rank p = 0,001)" ///
        5 "GMP corésident" 6 "GMP non corésident   -   (log-rank p = 0,587)" ///        
        7 "GPP corésident" 8 "GPP non corésident   -   (log-rank p = 0,019") rows(4) position (6) size(vsmall)) ///
...
angle(0)) /// Y-label values! Angle(0) rotates them.
xla(0(10)60) /// X-label values! From 0 to 6 years with 1 year intervals in between
    legend(order (1 "TM corésidente" 2 "TM non corésidente   -   (log-rank p = 0,068)"  ///
        3 "OM corésident" 4 "OM non corésident   -   (log-rank p = 0,000)" ///
        5 "TP corésident" 6 "TP non corésident   -   (log-rank p = 0,998)" ///        
        7 "OP corésident" 8 "OP non corésident   -   (log-rank p = 0,067)") rows(4) position (6) size(vsmall)) ///
...
angle(0)) /// Y-label values! Angle(0) rotates them.
xla(0(10)60) /// X-label values! From 0 to 6 years with 1 year intervals in between
    legend(order (1 "GMM corésident" 2 "GMM non corésident"  ///
        3 "GPM corésident" 4 "GPM non corésident" ///
        5 "GMP corésident" 6 "GMP non corésident" ///        
        7 "GPP corésident" 8 "GPP non corésident") rows(4) position (6) size(vsmall)) ///
		note("GMM : Grand-mère maternelle; GMP : Grand-mère paternelle" "GPM : Grand-père maternel; GPP :Grand-père paternel" , span size(vsmall)) 
		

graph save "Figure (gparents) 2 110521",replace 
graph export "Figure (gparents) 2 110521.png", replace width(2000)
graph export "Figure (gparents) 2 110521.tif", replace width(2000)
...
angle(0)) /// Y-label values! Angle(0) rotates them.
xla(0(10)60) /// X-label values! From 0 to 6 years with 1 year intervals in between
    legend(order (1 "TM corésidente" 2 "TM non corésidente"  ///
        3 "OM corésident" 4 "OM non corésident" ///
        5 "TP corésident" 6 "TP non corésident" ///        
        7 "OP corésident" 8 "OP non corésident") rows(4) position (6) size(vsmall)) ///
		note("TM : Tante maternelle; TP : Tante paternelle" "OM : Oncle maternel; OP : Oncle paternel" , span size(vsmall)) 
		
graph save "Figure (oncles_tantes) 2 110521",replace 
graph export "Figure (oncles_tantes) 2 110521.png", replace width(2000)
graph export "Figure (oncles_tantes) 2 110521.tif", replace width(2000)
	
graph combine "Figure (gparents) 2 110521.gph" "Figure (oncles_tantes) 2 110521.gph", row(1)

graph save "Figure (oncles_tantes_gp) 2 110521",replace 
graph export "Figure (oncles_tantes_gp) 2 110521.png", replace width(2000)
graph export "Figure (oncles_tantes_gp) 2 110521.tif", replace width(2000)

	
grc1leg "Figure (gparents) 2.gph" "Figure (oncles_tantes) 2 110521.gph", row(1) ring(0) span ///
note("Source: données des observatoires; calcul des auteurs.", span size(vsmall))
...
stset EventDate if residence==1, id(concat_IndividualId) failure(censor_death==1) ///
		time0(datebeg) origin(time DoB) exit(time DoB+(31557600000*5)+212000000) scale(31557600000)
bootstrap, rep(1000) seed(09051986) : stcox i.gender ib20.hdss_period i.MigDeadMO_MO_DTH_TVC i.MigDeadFA_FA_DTH_TVC ///
      ib7.birth_int_gp_YS_new ib0.res_Y_DTH_TVC  ///
	  ib4.MigDeadO_interv_new ib0.res_O_DTH_TVC ///
	  i.MigDeadTwin_Twin_DTH_TVC ib21.y3_mother_age_birth ///
	  ib0.coresidMGM##ib0.coresidMGF ///
...
**Modif 11032021
*Fairly shared model_ERC_080818
...
stcox i.gender ib20.hdss_period i.MigDeadMO_MO_DTH_TVC i.MigDeadFA_FA_DTH_TVC ///
	  ...
	  ib0.coresid_maunt ib0.coresid_muncle ///
	  ib0.coresid_paunt ib0.coresid_puncle  ///
	  , shared(MotherId) iter(10) forceshared

* Le shared fairtly model ne fonctionne que pour des échantillons pas trop grandes
* Stata ne supporte pas les  très grands échantillons et l'estimation prend du temps!
stcox i.gender if hdss_1==5 , shared(MotherId) iter(10) forceshared
XX
*est store u5
 ...
 **Coefplot relative to paternal gran-parents
 ...
 coefplot, xline(1) eform xtitle(Hazards Ratio (logged)) levels(95) ///
 ...
 **Coefplot relative to maternal grand-parents
 ...
 coefplot, xline(1) eform xtitle(Hazard Ratio (logged)) levels(95) ///
...
graph combine "p_grand_parents.gph" "m_grand_parents.gph", row(1) xcommon ycommon
