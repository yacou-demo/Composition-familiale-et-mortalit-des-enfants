...
* Correction (this actually fills in DoB after in previous file we made it .)
...
*** carren update- to check - replace datebeg with EventDate since it is cutting off important events before the ENU (22/6/2020)
*format datebeg %tC
sort concat_IndividualId EventDate /*datebeg*/
...
/*histogram EventDate if concat_IndividualId!=concat_IndividualId[_n-1] & CentreId=="UG011" ///
	& EventDate<clock("01may2005","DMY") & EventDate>clock("01jan2005","DMY"), xlabel(,angle(45)) bin(50) */
...
/*
    583,287  subjects
     41,541  failures in single-failure-per-subject data
  1,819,663  total analysis time at risk and under observation
...
log close
*/
tab CentreId if censor_death==1 & _st==1 //to count number of deaths in each hdss
/* Under-5 Deaths
...
------------+-----------------------------------
      Total |     41,393      100.00
*/
table CentreId [iw=_t-_t0], content(freq) format(%10.0f) missing row //to count person-years in each hdss 
